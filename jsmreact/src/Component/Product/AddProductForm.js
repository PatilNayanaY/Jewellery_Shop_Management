...
export default function AddProductForm(props) {
    ...
    //         let res = await fetch(`http://localhost:9090/saveProduct`, {
    //             method: "POST", headers:{
    ...
    //     e.preventDefault();
    //     await fetch("http://localhost:9090/allpCategory").then(respp => (respp.json()).then((data) => {setpcategory(data)}));
    // }
    return (
        <div className='container my-5'>
            {/* <form id='productAddForm' className="row g-3"  onSubmit={handleOnSubmit} > */}
            <form id='productAddForm' className="row g-3" action='http://localhost:9090/saveProduct' method='post' encType='multipart/form-data' onSubmit={handleOnSubmit} >
