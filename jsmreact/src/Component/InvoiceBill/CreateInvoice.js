...
export default function CreateInvoice(props) {
    ...
    //     }
    //     else fetch(`http://localhost:9090/api/employees/${empid}`).then(respp => respp.json()).then(data => setcurrentEmoData(data));
    //   }, [])

    const handleOnSubmit = async (e) => {
        e.preventDefault();
        try {
            let res = await fetch(`http://localhost:9090/invoice`, {
                method: "POST", headers: {
    ...
    const handleProduct = async (e) => {
        e.preventDefault();
        fetch("http://localhost:9090/allproduct").then(respp => respp.json()).then(data => {
            console.log(data)
            setproduct(data)
        });
